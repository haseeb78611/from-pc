...
class UploadSlidesScreen extends StatefulWidget {
  const UploadSlidesScreen({Key? key}) : super(key: key);

  @override
  State<UploadSlidesScreen> createState() => _UploadSlidesScreenState();
}

class _UploadSlidesScreenState extends State<UploadSlidesScreen> {
  ...
  uploadFile()async{
    path = 'slides/${semester[semesterNumber]}/${subject[subjectNumber]}/${fileNameController.text}.${pickedFile!.extension}';
    ...
    final url = await snapshot.ref.getDownloadURL().then((value) {
      ...
      String currentTime = timer.day.toString() + '-'+ timer.month.toString() + '-'+ timer.year.toString() + ' ${timer.hour}\:${timer.minute}\:${timer.millisecondsSinceEpoch}' ;
       database.ref(semesterNode[semesterNumber] + '_semester')
          ...
          .child(workType[workTypeNumber].toLowerCase())
          .child(materialTime[materialTimeNumber].toLowerCase())
                        ...
                        showCupertinoModalPopup(context: context, builder: (context) {
                          return Container(
                            ...
                            child: CupertinoPicker(
                                ...
                                children:  const [
                                  Center(child: Text('Lab')),
                                  Center(child: Text('Theory')),
                                ]
                            ),
                          );
                        },);
                        ...
                        showCupertinoModalPopup(context: context, builder: (context) {
                          return Container(
                            ...
                            child: CupertinoPicker(
                                ...
                                children:  const [
                                  Center(child: Text('Mid_Material')),
                                  Center(child: Text('Final_Material')),
                                ]
                            ),
                          );
                        },);
